 ...
 */
export function metersPreviewMeter(
  client: CreemCore,
  request: components.PreviewMeterApiRequestDto,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.MeterPreviewApiResponseDto,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.PreviewMeterApiRequestDto$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/meters/preview")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.MeterPreviewApiResponseDto$inboundSchema),
