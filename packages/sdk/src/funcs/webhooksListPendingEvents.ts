 ...
 */
export function webhooksListPendingEvents(
  client: CreemCore,
  id: string,
  ...
  Result<
    components.WebhookPendingEventListEntity,
    | CreemError
...
> {
  const input: operations.ListPendingWebhookEventsRequest = {
    id: id,
    limit: limit,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.ListPendingWebhookEventsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/webhooks/{id}/events/pending")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.WebhookPendingEventListEntity$inboundSchema),
