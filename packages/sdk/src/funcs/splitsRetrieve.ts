 ...
 */
export function splitsRetrieve(
  client: CreemCore,
  id: string,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SplitEntity,
    | CreemError
...
> {
  const input: operations.RetrieveSplitRequest = {
    id: id,
  };

  const parsed = safeParse(
    input,
    (value) => operations.RetrieveSplitRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/splits/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SplitEntity$inboundSchema),
