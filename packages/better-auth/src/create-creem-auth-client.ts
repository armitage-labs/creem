 ...
 *
 * @deprecated This wrapper is no longer needed. The `creemClient()` plugin now
 * provides full type inference (endpoint parameters and
 * `session.user.creemCustomerId`) through better-auth's server plugin
 * inference. Use standard `createAuthClient` directly:
 *
 * ```typescript
 * import { createAuthClient } from "better-auth/react";
