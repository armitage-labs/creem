...
- 374265d: Handle `subscription.scheduled_cancel` with an optional `onSubscriptionScheduledCancel` callback, persist the scheduled status and `cancelAtPeriodEnd`, and retain access only until the current period ends. Synchronize the cancellation flag when cancellation is undone or finalized.
...
- 0b9b661: Complete native Better Auth client inference and make checkout navigation explicit for 2.0.
