...
- 6ac6c3d: Bump creem SDK dependency to 1.11.0

## 0.6.2

### Patch Changes

- d54c988: Bump creem SDK dependency to 1.10.1
...
- a7f000c: Invoke `onRevokeAccess` with reason `subscription_canceled` when a `subscription.canceled` webhook arrives, before `onSubscriptionCanceled`. Scheduled cancellation events continue to leave access unchanged.
...
- b1a840e: Clarify webhook event names, access reasons, field casing, timestamps, checkout amounts, and metadata in the Next.js guide. Document optional checkout orders in callback JSDoc and link to the guide from the package README.

## 0.5.7

### Patch Changes

- 06a2b5a: Bump creem SDK dependency to 1.7.0
