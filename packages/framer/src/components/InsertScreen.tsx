...
// Fixed accent used only for the plugin's own UI chrome (selection checkboxes,
// "selected" badges, button preview). Colors on the INSERTED components are
// styled via Framer's native property controls, not baked in here.
const PLUGIN_ACCENT = '#FFBE98'
...
function ButtonConfiguration({ products, selectedId, setSelectedId, buttonText, setButtonText, checkoutType, setCheckoutType, selectedProduct }: ButtonConfigurationProps) {
              ...
              {searchedPairs.map(pair => {
                ...
                return (
                  <div
                    ...
                    role='listitem'
                  >
                    <input type='checkbox' checked={isSelected} readOnly className='h-4 w-4' style={{ accentColor: PLUGIN_ACCENT }} />
                    <div className='min-w-0 flex-1'>
                      <div className='flex items-center gap-2'>
                        <span className='truncate text-sm font-black text-black' title={pair.baseName}>
                          {pair.baseName}
                        </span>
                        ...
                        {isSelected && (
                          <span className='rounded px-1.5 py-0.5 text-[10px] font-black text-white' style={{ background: PLUGIN_ACCENT }}>
                            #{selectedIndex + 1}
                          </span>
                        )}
                      </div>
              ...
              {searchedProducts.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className='min-w-0 flex-1'>
                      <div className='flex items-center gap-2'>
                        <span className='truncate text-sm font-black text-black' title={p.name}>
                          {p.name}
                        </span>
            ...
            {selectedProducts.map((id, i) => {
              ...
              return (
                <div key={id} className='flex items-center gap-2 rounded-lg border-2 border-gray-300 bg-white px-3 py-2'>
                  ...
                  <span className='flex-1 truncate text-sm font-bold text-black' title={displayName}>
                    {displayName}
                  </span>
                      ...
                      <div className='flex w-full flex-col gap-1.5'>
                        {config.features.map((feature, featureIndex) => (
                          <div key={featureIndex} className='flex w-full items-center gap-1.5'>
                            <input
                              className={'text-creem-ink min-w-0 flex-1 rounded-lg border border-black bg-white px-3 py-2 text-sm font-semibold'}
                              value={feature}
                              onChange={e => {
