 ...
 */
export function affiliatesCreateInvite(
  client: CreemCore,
  request: components.CreateAffiliateInviteRequestEntity,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AffiliateInviteEntity,
    | CreemError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.CreateAffiliateInviteRequestEntity$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AffiliateInviteEntity$inboundSchema),
