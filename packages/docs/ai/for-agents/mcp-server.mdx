---
title: MCP Server
description: Give your AI agent 77 tools that call the Creem API directly, over the Model Context Protocol.
---

The Creem TypeScript SDK ships an [MCP](https://modelcontextprotocol.io) server. Point Claude Code, Claude Desktop, Cursor or any MCP client at it and your agent can create products, open checkouts, manage subscriptions, issue refunds, register webhook endpoints and inspect your store — by calling the real API, with your key.

<Tip>
  **Skill files vs MCP server.** [Skill files](/ai/for-agents/skill-files) are reference material
  your agent *reads* — they teach it how Creem works. The MCP server is a set of tools your agent
  *runs* — it changes live data. Most teams want both.
</Tip>
...
<Warning>
  **Test-mode keys require `--server test`.** Without it the SDK targets production, and every tool
  call fails with `401 Unauthorized — Invalid API Key`, even though your key is valid. This is the
  single most common setup problem.
</Warning>
...
77 tools, covering the API surface.
...
| **Customer credits** | 15    | `customer-credits-create-account`, `-get-account`, `-get-account-balance`, `-list-accounts`, `-list-entries`, `-credit-account`, `-debit-account`, `-freeze-account`, `-unfreeze-account`, `-reverse-transaction`, `-close-account`, `-post-transaction`, `-get-transaction`, `-reverse-transaction-by-id`, `-list-transactions-by-reference` |
| **Meters**           | 9     | `meters-create-meter`, `meters-get-meter`, `meters-list-meters`, `meters-update-meter`, `meters-archive-meter`, `meters-unarchive-meter`, `meters-preview-meter`, `meters-preview-existing-meter`, `meters-get-consumed-units`                                                                                                                |
| **Events**           | 3     | `events-ingest-events`, `events-preview-events`, `events-list-events`                                                                                                                                                                                                                                                                         |
| **Webhooks**         | 6     | `webhooks-list`, `webhooks-create`, `webhooks-get`, `webhooks-update`, `webhooks-delete`, `webhooks-get-secret`                                                                                                                                                                                                                               |
| **Affiliates**       | 5     | `affiliates-list`, `affiliates-retrieve`, `affiliates-list-commissions`, `affiliates-create-invite`, `affiliates-list-invites`                                                                                                                                                                                                                |
| **Splits**           | 4     | `splits-create`, `splits-list`, `splits-retrieve`, `splits-delete`                                                                                                                                                                                                                                                                            |
...
<Warning>
  A full-access API key grants an agent **every** tool, including destructive ones —
  `transactions-refund`, `subscriptions-cancel`, `products-archive`, `discounts-delete`,
  `webhooks-delete` and `customer-credits-debit-account`. Scoped keys (for example `products:read`
  or `webhooks:write`) limit which tools succeed; a tool the key is not scoped for fails with `403`.
</Warning>
...
| `--tool`       | Mount only this tool; repeatable                | all 77  |
...
<Accordion title="The server doesn't start">
  Check `node --version` — v22 or newer is required. Then run the command directly in a terminal;
  MCP clients often swallow startup errors.
</Accordion>

<Accordion title="My agent can't see the tools">
  Restart the MCP client after editing its config — most read it only at launch. Then confirm the
  server is listed and connected in your client's MCP settings.
</Accordion>
