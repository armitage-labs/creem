...
- **Daily billing** - Charged every day
...
- **Custom billing** - Charged on any other cadence, such as weekly or every 2 weeks

For a custom cadence, set `billing_period` to `custom` and provide `recurring_interval` (`day`, `week`, `month`, or `year`) and `recurring_interval_count`. See [Create Product](/api-reference/endpoint/create-product).
...
<CardGroup cols={2}>
  ...
  >
    Learn how to set up free or paid trial periods for your subscriptions.
  </Card>
