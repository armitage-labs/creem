...
const grantAccessEvents = new Set([
  'subscription.active',
  'subscription.trialing',
  'subscription.paid',
]);

const revokeAccessEvents = new Set(['subscription.paused', 'subscription.expired']);

// In your webhook endpoint
export async function handleWebhook(request: Request) {
  try {
    const rawBody = await request.text();
    const event = await constructWebhookEvent<SubscriptionEntity>(rawBody, request.headers, {
    ...
    if (grantAccessEvents.has(event.type)) {
      await db.user.update({
        where: { id: userId },
        data: {
          subscriptionActive: true,
      ...
      console.log(`Granted access for subscription ${event.data.id}`);
    }

    if (revokeAccessEvents.has(event.type)) {
      await db.user.update({
        where: { id: userId },
        data: { subscriptionActive: false },
      });

      console.log(`Revoked access for subscription ${event.data.id}`);
    }
