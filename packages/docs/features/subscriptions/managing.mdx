...
<Warning>
  The dashboard doesn't ask for confirmation or show a warning when you edit a price, and Creem
  doesn't notify existing subscribers about price changes. If you plan to move subscribers to a new
  price, let them know in advance.
</Warning>

### Raising Prices for Existing Subscribers

Creem has no built-in flow for moving existing subscribers to a new price. Instead, create a product with the new price, then run a one-off script that:

1. Pages through your subscriptions with the [search subscriptions endpoint](/api-reference/endpoint/search-subscriptions). The endpoint has no product or status filter, so filter the results in your script.
2. Moves each matching subscription to the new product with the [upgrade subscription endpoint](/api-reference/endpoint/upgrade-subscription).
...
const OLD_PRODUCT_ID = "prod_OLD_PRICE";
const NEW_PRODUCT_ID = "prod_NEW_PRICE";

// Collect active subscriptions on the old product (100 per page)
const toMigrate: string[] = [];
for await (const page of await creem.subscriptions.search(1, 100)) {
  for (const sub of page.result.items) {
    const productId = typeof sub.product === "string" ? sub.product : sub.product.id;
    if (productId === OLD_PRODUCT_ID && sub.status === "active") {
      toMigrate.push(sub.id);
    }
  }
}

// Move each subscription to the new product
for (const subscriptionId of toMigrate) {
  await creem.subscriptions.upgrade(subscriptionId, {
...
```bash
# Step 1: Fetch subscriptions page by page (up to 100 per page), then keep
# the ones whose product.id is the old product and whose status is active
curl -X GET "https://api.creem.io/v1/subscriptions/search?page_number=1&page_size=100" \
  -H "x-api-key: YOUR_API_KEY"

# Step 2: Move each matching subscription to the new product
curl -X POST https://api.creem.io/v1/subscriptions/sub_YOUR_SUBSCRIPTION_ID/upgrade \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
