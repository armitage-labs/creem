...
if (event.type === 'subscription.canceled') {
  const customer =
    typeof event.data.customer === 'string' ? undefined : event.data.customer;

  console.log(`Subscription ${event.data.id} canceled`);
...
if (['subscription.canceled', 'subscription.paused', 'subscription.expired'].includes(event.type)) {
