...
<Tip>
  Post the balanced pair (or set) that describes the whole event in one call rather than a separate credit and debit — a retry or a crash between two calls can leave one side written and the other not.
</Tip>
