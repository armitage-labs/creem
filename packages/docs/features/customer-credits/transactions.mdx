...
<Info>
  ...
  `insufficient_balance` error. Check the balance first if you want to show your users a friendly
  message.
</Info>
...
## Multi-entry transactions — move credits between accounts

Credit and debit touch one account at a time. When one business event should move credits between accounts atomically — a transfer between two customers, or a bundle that debits a pool and credits several sub-accounts — post a single transaction with several entries. Entries must balance: total debits must equal total credits, and a transaction needs at least two entries.

```bash
curl -X POST https://api.creem.io/v1/customer-credits/transactions \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "reference": "transfer_42",
    "idempotency_key": "transfer_42",
    "entries": [
      { "account_id": "cca_7kXmR2pQ9vN", "side": "debit", "amount": "300" },
      { "account_id": "cca_2bQpL5sT8xM", "side": "credit", "amount": "300" }
    ]
  }'
```
...
<Tip>
  Post the balanced pair (or set) that describes the whole event in one call rather than a separate
  credit and debit — a retry or a crash between two calls can leave one side written and the other
  not.
</Tip>
...
```bash
curl https://api.creem.io/v1/customer-credits/transactions/{transaction_id} \
  -H "x-api-key: YOUR_API_KEY"
```

### Reverse a transaction by ID

Reverses the whole transaction — every entry — without naming the account. Use this when you hold the transaction ID from an earlier response and the transaction spans several accounts.

```bash
curl -X POST https://api.creem.io/v1/customer-credits/transactions/{transaction_id}/reverse \
  -H "x-api-key: YOUR_API_KEY"
```
...
Because you set `reference` on each write, you can list everything Creem recorded for one event in your system — an order, a campaign, a support ticket.

```bash
curl "https://api.creem.io/v1/customer-credits/transactions?reference=order_789" \
  -H "x-api-key: YOUR_API_KEY"
```
  ...
  "data": [
    {
      ...
      "idempotency_key": "reward_order_789",
      "reversal_of": null,
      "entries": [{ "side": "credit", "amount": "500", "account_id": "cca_7kXmR2pQ9vN" }]
    }
  ],
  "has_more": false
}
...
<Tip>
  Use a key that's tied to the business event, like `reward_order_789` or `topup_sub_123_2026-04`.
  This way, even if your code accidentally fires twice for the same order, the customer only gets
  credited once.
</Tip>
