...
| `usage_prices[].unit_price`      | Tax-exclusive price per **single** meter unit, in minor units of the product currency. "$20 per 1,000,000 tokens" is `0.002`, not `20`.                           |
...
| `features[].customer_credits`    | The grant a purchase makes: `amount` (as a string), an optional `unit_label`, and the `bucket_name` it funds. Point it at the same bucket the usage price debits. |
