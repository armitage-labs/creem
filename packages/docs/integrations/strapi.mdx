...
- **Billing interval** for subscriptions (monthly, every 3 months, every 6 months, yearly)
...
2. Add your public HTTPS endpoint:
   ...
   **Local development** (Creem cannot reach `localhost` directly, so expose your local Strapi with a tunnel first):
   ...
   ```
   https://abcd.ngrok.io/api/strapi5-plugin-for-creem/webhook
   ```
...
<CardGroup cols={2}>
  <Card title="strapi5-plugin-for-creem on npm" icon="npm" href="https://www.npmjs.com/package/strapi5-plugin-for-creem">
