...
<AccordionGroup>
  ...
  <Accordion title="Does Creem support weekly billing?">
    Yes. Creem supports **daily**, **monthly**, **3-month**, **6-month**, and **yearly** billing cycles as presets. For any other cadence, such as weekly, set `billing_period` to `custom` and provide `recurring_interval` (`day`, `week`, `month`, or `year`) and `recurring_interval_count`. For example, `recurring_interval: "week"` with `recurring_interval_count: 1` bills weekly. See [Create Product](/api-reference/endpoint/create-product).
  </Accordion>
