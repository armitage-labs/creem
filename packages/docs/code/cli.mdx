---
title: Creem CLI
description: Install, configure, and use the Creem CLI to manage your business from the terminal. Browse interactively or automate your workflows.
---
...
Search records, open details, and manage subscriptions from the terminal. See the [interactive guide](/ai/for-humans/cli#interactive-browsing) for navigation and examples.

## Local webhooks

`creem listen` delivers your store's webhooks to a local URL with no tunnel and no deploy. It creates a temporary endpoint in `cli` delivery mode, forwards every event with the exact body and `creem-signature` header a real delivery carries, reports the status your server returned, and deletes the endpoint when you stop it.

```sh
creem listen --forward-to http://localhost:3000/api/webhooks
creem listen --forward-to http://localhost:3000/api/webhooks --events checkout.completed,subscription.paid
```

The signing secret is printed at startup so you can verify signatures exactly as in production. Trigger events with test checkouts, or use **Send test event** on the endpoint in the dashboard. Failed forwards are recorded in the dashboard event log with the local status; nothing is retried, so replay from the dashboard once your handler is fixed.

Pending events are available for 24 hours. Disabled endpoints return no pending events. Dashboard replays queue a fresh event only for the selected CLI endpoint.

The first acknowledgment of an event is final; repeated acknowledgments return the stored result. With `--json`, an event record's `status_code` describes this listener's local HTTP response, while `success` and the final `failed` count reflect the stored acknowledgment. These can differ when multiple listeners share an endpoint. Use separate CLI endpoints for independent listeners.

For direct polling, `creem webhooks pending <id> --limit <count>` accepts an integer from 1 to 100 (default 50).

- Listening in live mode requires `--live`.
- `--webhook <id>` reuses a CLI-mode endpoint you manage yourself (it is never deleted); `--once` drains what is pending and exits.
- `--json` prints one NDJSON record per event for scripts and agents.
...
creem webhooks create --url https://example.com/webhooks/creem --event checkout.completed --json
```
...
`subscriptions list` now lists subscriptions directly instead of deriving them from transactions; the old transaction-derived `note` field is removed. `discounts list` now supports search filters and pagination. Scripts invoking destructive actions must supply `--yes`.

See the [agent workflow](/ai/for-agents/cli) or [interactive guide](/ai/for-humans/cli) for your preferred interface.
