...
A debit through the API that exceeds the balance doesn't fire this event. That request fails with `422` and the error code `insufficient_balance`.
...
An empty credit account was topped up with an automatic charge. It fires only for customers who opted in to auto-recharge, after the charge succeeds and the credits are added. The same top-up also fires `credits.granted`.
