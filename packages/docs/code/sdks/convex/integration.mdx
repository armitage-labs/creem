...
{/*
  INTENTIONALLY NOT IN docs.json NAVIGATION.

  This page is written for AI agents, not for sidebar browsing: it repeats the
  whole setup sequence in one self-contained page, which would duplicate the
  human-facing Quickstart → Subscriptions → Entitlements path if it sat beside
  them in the sidebar. Agents reach it through `llms.txt`, which links it
  directly, and through the in-page links from Quickstart, Advanced, and
  Migration.

  Do not "fix" this by adding it to `docs.json` — that is a deliberate
  exception to the repo rule in `packages/docs/CLAUDE.md`. Keep the page
  self-contained so an agent that lands here needs no other page.
*/}
