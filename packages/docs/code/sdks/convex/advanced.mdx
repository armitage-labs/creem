...
Authentication and application roles remain your responsibility. The generated API provides the billing authorization baseline: it derives the billing entity from `resolve`, scopes customer data to that entity, and verifies explicitly selected subscriptions before reading or changing them.

**Quick start.** `creem.api({ resolve })` generates ready-to-export Convex functions. Each one calls your `resolve` callback to authenticate and determine the `entityId`; the billing entity is derived from the session, never from client input. For team billing, verify membership and the required billing role before returning an organization ID. This is what the [Quickstart](/code/sdks/convex/quickstart) uses.
...
{ kind: "app-plan", freePlanId,         updateBehavior?: "period-end" | "immediate"   }
```
...
Pass `i18n` at the provider, or per widget, to replace default UI labels and format dates and currency. It covers cards, dialogs, billing history, portal buttons, recovery banners, credits, and accessibility labels:
