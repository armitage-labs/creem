...
```bash npm
npm install @creem_io/better-auth better-auth@^1.5.6
```
...
```bash yarn
yarn add @creem_io/better-auth better-auth@^1.5.6
```

```bash bun
bun add @creem_io/better-auth better-auth@^1.5.6
```

</CodeGroup>

Better Auth `^1.5.6` is required for native endpoint type inference. The Creem TypeScript SDK is
included as a dependency of the plugin.
...
The client plugin infers endpoint inputs and `session.user.creemCustomerId` through Better Auth.
...
to checkout metadata as `referenceId`. Navigation is manual by default; pass `redirect: true`
to let Better Auth navigate automatically.
