...
| Reachable customer support email        | Ensure you have a reachable email for [customer support](#customer-experience-and-support) which will be displayed on the email receipt customers receive |
| AI content moderation                   | If your product uses AI to generate images, video, or audio, you must have [content filters](#content-safety-%26-moderation-requirements) that effectively prevent NSFW and sexually explicit outputs |
| Compliance with prohibited product list | Your product is not included in our [prohibited product list](#prohibited-and-restricted-products)                                                 |
...
All AI-powered generation tools — including image, video, and audio products — must implement robust content moderation to prevent the creation of prohibited content.
...
The following open-source/open-weight models lack built-in NSFW content filters. Platforms deploying these models must implement their own robust content filtering — otherwise they will be rejected.

<Note>
  All versions and community derivatives of the listed models are covered by this policy. Platforms using **censored, API-gated versions** (e.g., Stability AI's official API with safety filters, DALL-E, Midjourney, Runway) are not affected — the restriction applies to self-hosted or open-weight deployments without adequate content filtering.
</Note>
