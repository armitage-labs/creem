...
1. Querying subscription details
    ...
    - Retrieve the Product ID and Item ID from the subscription item.
...
<Accordion icon="location-crosshairs" title="Parse the response">
 Parse, locate and store the Product Id and Item ID of the subscription item, located under the items array. 
...
2. Make the subscription change
    - With the Product ID, Item Id and subscription ID, call the [UPDATE subscription API](https://docs.creem.io/api-reference/endpoint/get-subscription).
