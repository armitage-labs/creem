...
<Info>
  When using the <a href="/sdk/nextjs-adapter">@creem_io/nextjs adaptor</a>, set a `successUrl` on `<CreemCheckout />` (or `defaultSuccessUrl` on your `Checkout` route) to control where Creem redirects users—no manual query string handling required.
</Info>
...
### Handle return URLs in Next.js

Use a server component or Route Handler to read search params, verify the signature, and then show the success UI:
...
const verifySignature = (params: RedirectParams, apiKey: string) => {
...
export default function ThankYouPage({
  ...
  const params = {
    checkout_id: searchParams.checkout_id as string | undefined,
    order_id: searchParams.order_id as string | undefined,
    customer_id: searchParams.customer_id as string | undefined,
    subscription_id: searchParams.subscription_id as string | undefined,
    product_id: searchParams.product_id as string | undefined,
    request_id: searchParams.request_id as string | undefined,
    signature: searchParams.signature as string | undefined,
  };

  const isValid = verifySignature(params, process.env.CREEM_API_KEY!);
  if (!isValid) {
    redirect("/error");
  }
  ...
  return (
    <div>
      ...
      <p>Checkout ID: {params.checkout_id}</p>
    </div>
  );
}
```
