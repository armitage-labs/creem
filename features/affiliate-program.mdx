...
```mermaid
flowchart LR
    A[Merchant creates program] --> B[Invites affiliates]
    B --> C[Affiliate accepts invite]
    C --> D[Shares referral link]
    D --> E[Customer clicks link]
    E --> F[Cookie tracks referral]
    F --> G[Customer purchases]
    G --> H[Commission credited]
    H --> I[Affiliate requests payout]
```
...
<Frame>
  <img
    style={{ borderRadius: '0.5rem' }}
    src="/images/affiliate/05-growth-menu-expanded.png"
